...
while ($row = $result_chinese->fetch_assoc()) {
    $data['chinese'][] = $row;
    $data['total_chinese_credits'] += $row['credit'];
}
$data['total_chinese_credits'] = min($data['total_chinese_credits'], 4);
...
while ($row = $result_junior_english->fetch_assoc()) {
    ...
    $data['total_junior_english_credits'] += $row['credit'];
}
$data['total_junior_english_credits'] = min($data['total_junior_english_credits'], 4);
...
while ($row = $result_senior_english->fetch_assoc()) {
    ...
    $data['total_senior_english_credits'] += $row['credit'];
}
$data['total_senior_english_credits'] = min($data['total_senior_english_credits'], 2);
//查詢校訂博雅
$sql_school_general = "
    SELECT c.course_ID, c.course_name, c.dept_name, c.credit
    FROM course c
...
while ($row = $result_school_general->fetch_assoc()) {
    ...
    $data['total_school_general_credits'] += $row['credit'];
}
//博雅
$sql_general = "
...
while ($row = $result_general->fetch_assoc()) {
    $data['general'][] = $row;
    $data['total_general_credits'] += $row['credit'];
...
while ($row = $result_required->fetch_assoc()) {
    $data['required'][] = $row;
    $data['total_required_credits'] += $row['credit'];
}
//系內
$sql_elective = "
...
while ($row = $result_elective->fetch_assoc()) {
    $data['elective'][] = $row;
    $data['total_elective_credits'] += $row['credit'];
...
while ($row = $result_language->fetch_assoc()) {
    $data['language'][] = $row;
    $data['total_language_credits'] += $row['credit'];
}
$sql_other = "
	SELECT c.course_ID, c.course_name, c.dept_name, c.credit
	FROM course c
...
while ($row = $result_language->fetch_assoc()) {
    $data['other'][] = $row;
    $data['total_other_credits'] += $row['credit'];
}
